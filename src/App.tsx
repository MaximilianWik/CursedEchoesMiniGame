...
type BossRuntime = {
  ...
  currentHp: number;
  phaseIdx: number;
  nextAttackAt: number;          // performance.now time of next scheduled attack
  nextPhraseAt: number;          // performance.now time when the next phrase may spawn
  patternRotationIdx: number;    // rotates through phase.patterns
  enraged: boolean;
...
export default function App() {
  ...
  const enterBoss = useCallback((bossId: string) => {
    ...
    bossRef.current = {
      ...
      currentHp: def.maxHp,
      phaseIdx: 0,
      nextAttackAt: performance.now() + 1800,     // brief grace period on entry
      nextPhraseAt: performance.now() + 600,      // first phrase shows quickly
